...
| `skip-token-revoke` | If true, skip revoking the GitHub token on exit. If false, revocation runs only when Vault issued a lease for the secret read (non-empty lease_id); an empty lease_id means the vault-plugin-secrets-github token-cache path and revocation is skipped so shared tokens are not invalidated. In that case a workflow notice explains that revocation was skipped. | `false`  | `true`  |
<!--/inputs-->
## Outputs
<!--outputs-->
| Name    | Description                           |
